...
// Creo una caja (objeto) vacía donde apuntaré cuántas veces sale cada sonido.
const soundCounts = {};

// Recorro la lista de usuarios uno a uno.
for (const user of usersSounds2) {
    // Para cada usuario, miro dentro de sus sonidos favoritos.
    for (const soundName in user.favoritesSounds) {
        // 'soundName' es el nombre del sonido (ej: 'waves', 'rain').

        // Pregunto: ¿Ya tengo este sonido apuntado en mi caja?
        if (soundCounts[soundName]) {
            // Si ya está, le sumo 1 al contador.
            soundCounts[soundName]++;
        } else {
            // Si no estaba, lo apunto por primera vez con el valor 1.
            soundCounts[soundName] = 1;
        }
    }
}

// Muestro la lista final con el recuento de sonidos.
console.log('Ejercicio 11 - Conteo Sonidos:', soundCounts);
